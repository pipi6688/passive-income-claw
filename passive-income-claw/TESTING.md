...
| Phase | 类型 | 方式 | 状态 |
|-------|------|------|------|
| 0 | 自动化单元测试（纯函数） | `node --test` | ✅ 55 tests 通过 |
| 1 | 脚本 CLI 测试（I/O + 参数解析） | 手动命令行 | ⬜ 待测 |
| 2 | OpenClaw 集成测试（LLM 完整流程） | 手动对话 | ⬜ 待测 |
| 3 | 边界条件测试 | 手动 | ⬜ 大部分已被 Phase 0 覆盖，剩余待测 |
...
- [ ] 所有 7 个脚本的基本功能正常
- [ ] 错误情况输出 JSON 到 stderr，exit code 非零
- [ ] positional 和 --flag 两种参数写法都能用
